...
def colour_segmentation(img, num_segments=1000, round_schedule = [0.02, 0.04, 0.06, 0.08], colour_median_prop=0, max_clust_size=0.05, min_clust_size=0.002):
    ...
    for clust in range(len(clust_sizes)):
        if clust_sizes[clust] > max_clust_prop or clust_sizes[clust] < min_clust_prop: # background or noise resp.
...
if __name__ == '__main__':
    ...
    train_imgs = dl.get_train_images_and_classes(lambda img, y, meta: img)

    train_x = train_imgs['x']
    train_y = train_imgs['y']
    train_meta = train_imgs['meta']
    ...
    template = preprocessing.build_template(train_x, train_meta)
    
    for idx_idx in range(len(train_x)):
        idx = train_idxs[idx_idx]
        
        # Load image
        img = train_x[idx]()
        imgclass = train_y[idx]
        if 'bounding_boxes' in train_meta[idx]:
            imgboxes = train_meta[idx]['bounding_boxes']
        else:
            imgboxes = []
        ...
        lower = lambda centroid, dim: max(centroid[dim] - bbox_size/2.0, 0)
        upper = lambda centroid, dim: min(centroid[dim] + bbox_size/2.0-1, img.shape[dim])
        ...
        num_impossible_here = sum(max(bbox['width'], bbox['height'])*overlap_ratio-1 >= bbox_size for bbox in imgboxes)
        ...
        if idx_idx%50 == 49:
            box_precision = 100*tp_boxes / float(num_boxes)
            fish_recall = 100*tp_fish / float(num_fish)
    ...
    print("\nTotal box precision: %g%% (%d/%d)\nTotal fish recall: %g%% (%d/%d%s)\n" % (box_precision, tp_boxes, num_boxes, fish_recall, tp_fish, num_fish, (", %d impossible" % num_impossible) if num_impossible > 0 else ""))
